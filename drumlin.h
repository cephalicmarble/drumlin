...
public:
    ...
    string_list():base(){}
    string_list(std::initializer_list<string> init):base(init){}
    ...
    base::size_type count()
    {
        return std::distance(begin(),end());
    }
    friend ostream &operator<<(ostream &stream,string_list &vecS);
};
...
typedef std::map<string,string> dict_type;
extern string substitute(string tpl,dict_type const& dict,string delim = "%");
